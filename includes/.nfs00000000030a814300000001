...
#define RPL_WELCOME(user_id, nickname) \
	(LOCAL + "001 " + nickname + " :Welcome to the Internet Relay Network " + user_id + "\r\n")
#define RPL_YOURHOST(client, servername, version) \
	(LOCAL + "002 " + client + " :Your host is " + servername + " (localhost), running version " + version + "\r\n")
#define RPL_CREATED(client, datetime) \
	(LOCAL + "003 " + client + " :This server was created " + datetime + "\r\n")
#define RPL_MYINFO(client, servername, version, user_modes, chan_modes, chan_param_modes) \
	(LOCAL + "004 " + client + " " + servername + " " + version + " " + user_modes + " " + chan_modes + " " + chan_param_modes + "\r\n")
#define RPL_ISUPPORT(client, tokens) \
	(LOCAL + "005 " + client + " " + tokens + " :are supported by this server\r\n")

// Error messages
#define RPL_INVITING(nickname, target) \
	(LOCAL + "341 " + nickname + " " + target + " :Inviting user to channel\r\n")
#define ERR_NOSUCHCHANNEL(client, channel) \
	(LOCAL + "403 " + client + " #" + channel + " :No such channel\r\n")
#define ERR_UNKNOWNCOMMAND(client, command) \
	(LOCAL + "421 " + client + " " + command + " :Unknown command\r\n")
#define ERR_NOTONCHANNEL(client, channel) \
	(LOCAL + "442 " + client + " #" + channel + " :You're not on that channel\r\n")
#define ERR_USERONCHANNEL(client, nick, channel) \
	(LOCAL + "443 " + client + " " + nick + " #" + channel + " :is already on channel\r\n")
#define ERR_NEEDMOREPARAMS(client, command) \
	(LOCAL + "461 " + client + " " + command + " :Not enough parameters.\r\n")
...
#define ERR_CHANOPRIVSNEEDED(client, channel) \
	(LOCAL + "482 " + client + " #" + channel + " :You're not channel operator\r\n")
...
#define ERR_NOSUCHSERVER(client, servername) \
	(LOCAL + "402 " + client + " " + servername + " :No such server\r\n")
#define ERR_NOMOTD(client) \
	(LOCAL + "422 " + client + " :MOTD File is missing\r\n")
#define RPL_MOTDSTART(client, servername) \
	(LOCAL + "375 " + client + " :- " + servername + " Message of the day - \r\n")
#define RPL_MOTD(client, motd_line) \
	(LOCAL + "372 " + client + " :" + motd_line + "\r\n")
#define RPL_ENDOFMOTD(client) \
	(LOCAL + "376 " + client + " :End of /MOTD command.\r\n")

// NAMES
#define RPL_NAMREPLY(client, symbol, channel, list_of_nicks) \
	(LOCAL + "353 " + client + " " + symbol + " #" + channel + " :" + list_of_nicks + "\r\n")
#define RPL_ENDOFNAMES(client, channel) \
	(LOCAL + "366 " + client + " #" + channel + " :End of /NAMES list.\r\n")

// NICK
#define ERR_NONICKNAMEGIVEN(client) \
	(LOCAL + "431 " + client + " :There is no nickname.\r\n")
#define ERR_ERRONEUSNICKNAME(client, nickname) \
	(LOCAL + "432 " + client + " " + nickname + " :Erroneous nickname\r\n")
#define ERR_NICKNAMEINUSE(client, nickname) \
	(LOCAL + "433 " + client + " " + nickname + " :Nickname is already in use.\r\n")
